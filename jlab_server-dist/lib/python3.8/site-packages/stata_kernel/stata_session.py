...
class StataSession():
    ...
    def do(self, text, md5, **kwargs):
        ...
        #logging.debug("STATA SESSION DO")
        #logging.debug(text)
        #logging.debug(','.join('{0}={1!r}'.format(k,v) for k,v in kwargs.items()))
    ...
    def expect(self, text, child, md5, text_to_exclude=None, display=True):
        ...
        Args:
            ...
            text_to_exclude (str): string of text to exclude from output. It is
                ...
                on \\n in `expect`.
        """
        #logging.debug("EXPECT display="+str(display))
        ...
        # @BEN: from https://githubmemory.com/repo/kylebarron/stata_kernel/issues/391
        ...
        while match_index != 0:
            #logging.debug(child)
            #logging.debug(expect_list)
            match_index = child.expect(expect_list, timeout=None)
            res = child.before
            #logging.debug("match_index="+str(match_index))
            #logging.debug(child)
            if match_index == 0:
                break
            if match_index == 1:
    ...
    def expect_graph(self, child, res):
        ...
        # @BEN: Stata 17 mod
        fname = re.search(r'/(graph\d+\.\w+) (saved|written)', res).group(1)
