...
function __julia_py_repr_matrix(obj)
  ...
  # @benbenz: doesnt work anymore
  # Feather.write(tf, convert(DataFrame, obj))
  Feather.write(tf, DataFrame(obj,:auto))
                ...
                try:
                    ...
                    feather.write_dataframe(
                        pandas.DataFrame(
                            obj, columns=map(str, range(obj.shape[1]))),
                        feather_tmp_)
                # @ben : latest Julia packages versions throw errors here ...
                # return 'convert(Matrix, Feather.read("' + feather_tmp_ + '"))'
                return 'Matrix(Feather.read("' + feather_tmp_ + '"))'
