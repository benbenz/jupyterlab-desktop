...
class sos_Julia:
    ...
    supported_kernels = {'Julia': ['julia-?.?','julia']}
...
#
    ...
    def put_vars(self, items, to_kernel=None):
        self.sos_kernel.warn(f'SoS-Julia:put_vars:items={items}')
        self.sos_kernel.warn(f'SoS-Julia:put_vars:to_kernel={to_kernel}')
        ...
        self.sos_kernel.warn(f'SoS-Julia:put_vars:items post markdown={items}')

        if not items:
            self.sos_kernel.warn(f'SoS-Julia:put_vars:PREMATURE leave') 
            return {}

        res = {}
        for item in items:
            self.sos_kernel.warn(f'SoS-Julia:put_vars:Processing item {item}')
            while True:
                #
                ...
                self.sos_kernel.warn(f'SoS-Julia:put_vars:py_repr={py_repr}')

                k_response = self.sos_kernel.get_response(
                    py_repr, ('execute_result',))

                self.sos_kernel.warn(f'SoS-Julia:put_vars:k_response={k_response}')

                response = k_response[0][1]

                self.sos_kernel.warn(f'SoS-Julia:put_vars:response={response}')

                expr = response['data']['text/plain']

                self.sos_kernel.warn(f'SoS-Julia:put_vars:EXPRE = {expr}')
            ...
            try:
                ...
                self.sos_kernel.warn('DONE eval')
            except Exception as e:
                self.sos_kernel.warn('Failed to evaluate {!r}: {}'.format(
                    expr, e))
                self.sos_kernel.warn(f'Returning None')
                return None
        self.sos_kernel.warn(f'RES = {res}')
        return res
